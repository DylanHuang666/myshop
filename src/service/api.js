...
export async function regist(params){
  return await axios.post(baseURL + '/user/registUser', params);
}

//用户登录 userName password
export async function login(params){
  return await axios.post(baseURL + '/user/loginUser', params);
}

//获取产品的类型
export async function getProductType(){
  return await axios.get(baseURL + '/type/getProductType');
}

//获取产品  typeId  start  limit
export async function getProductsByType(params){
  return await axios.get(baseURL + '/product/getProductsByType', {params});
}

//获取推荐产品（mock的数据）
export async function getRecommendProducts(){
  return await axios.get('/mock/product/getRecommendProducts');
}

//获取产品详情信息
export async function getDetail(params){
  return await axios.get(baseURL + '/product/getDetail', {params});
}

//加入购物车  userId productId
export async function addCart(params){
  return await axios.post(baseURL + '/cart/addCart', params);
}

//获取用户购物车信息  userId
export async function getCart(params){
  return await axios.get(baseURL + '/cart/getCart', {params});
}

//上传用户头像  userId   avatar
export async function uploadUserImg(params){
  return await axios.post(baseURL + '/user/uploadUserImg', params);
}

//获取用户信息  userId
export async function getUserInfo(params){
  return await axios.get(baseURL + '/user/getUserInfo', {params});
}
